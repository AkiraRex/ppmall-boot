...
import com.ppmall.service.IOAuthService;
...
public class AuthorizeController {
	...
	public Object authorize(HttpServletRequest request) throws URISyntaxException, OAuthSystemException {
		...
		try {
			...
			// 检查传入的客户端id是否正确
			if (!iOAuthService.checkClientId(oauthRequest.getClientId())) {
			...
			if (!subject.isAuthenticated()) {
				if (!login(subject, request)) {// 登录失败时跳转到登陆页面
					...
					// iClientService.findByClientId(oauthRequest.getClientId()));
			...
			if (responseType.equals(ResponseType.CODE.toString())) {
				...
				iOAuthService.addAuthCode(authorizationCode, username);
			}
